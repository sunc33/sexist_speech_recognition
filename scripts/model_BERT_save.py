...
def init_model():
    ...
    model_ok.compile(optimizer='adam',loss='binary_crossentropy',metrics=['accuracy',metrics.Precision(),metrics.Recall()])
...
def load_trained_model():
    ...
    initialise the model with the trained weights
    """
    path = os.path.join(os.path.dirname(__file__), "../sexist_speech_recognition/model/model_weights.h5")

    model_testing = init_model()
    model_testing.load_weights(path)
    return model_testing

def pred_with_load_model(model, tweet_preproc):
    """
